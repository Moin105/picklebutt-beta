...
window.db = db;
window.dataRef = ref(db, "pickleLand/" + name);
window.set = set;
 function updateData(name, newData) 
{
  
  const db = getDatabase();
  const dataRef = ref(db, "pickleLand/" + name);
  get(dataRef)
  .then((snapshot) => {
    if (snapshot.exists()) {
      ...
      if(userData.Score < newData.Score){
         ...
         .then(() => {
           ...
           // Call fetchDataAndAssignTop10 after updating the data
...
async function fetchData() {
  const dataRef = ref(db, 'pickleLand/');
...
async function fetchDataAndAssignTop10() {
  ...
  console.log(window.fetchData); // Check if top10 is assigned
}
