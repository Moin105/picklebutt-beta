...
const landMessages = [
  ...
  ['Let @Donin001 know his bud is safe!'],
...
function preload() {
  picklebuttThrustAnimation = loadAnimation(...getAssetList('spacebudz_thrust', 10));
  picklebuttThrustAnimation.frameDelay = 3;
  picklebuttThrustOutAnimation = loadAnimation(...getAssetList('spacebudz_thrustout', 4));
  picklebuttThrustOutAnimation.looping = false;
  cloudAnimation = loadAnimation(...getAssetList('stars', 19));
  picklebuttImage = loadImage(`${basePath}assets/spacebudz_normal.png`);
...
function setup() {
  ...
  moon = createSprite();
  moon.maxSpeed = 0;
  moon.setCollider('circle', 0, 0, 238);
...
async function drawMessages() {
  if (paused) {
    ...
    text(
      gameOver
        ? `Your bud ${splashDown ? 'missed the moon lmao' : `${tooFastToLand ? 'hit the moon too fast' : ''
