...
function preload() {
  picklebuttThrustAnimation = loadAnimation(...getAssetList('dexyyy_thrust', 4));
  picklebuttThrustAnimation.frameDelay = 3;
  picklebuttThrustOutAnimation = loadAnimation(...getAssetList('dexyyy_thrustout', 4));
  picklebuttThrustOutAnimation.looping = false;
  cloudAnimation = loadAnimation(...getAssetList('stars', 19));
  picklebuttImage = loadImage(`${basePath}assets/dexyyy_normal.png`);
...
async function drawMessages() {
  if (paused) {
    ...
    text(
      gameOver
        ? `Dexyyy ${splashDown ? 'missed the moon lmao' : `${tooFastToLand ? 'hit the moon too fast' : ''
