...
function preload() {
  picklebuttThrustAnimation = loadAnimation(...getAssetList('jelly_thrust', 10));
  picklebuttThrustAnimation.frameDelay = 1;
  picklebuttThrustOutAnimation = loadAnimation(...getAssetList('jelly_thrustout', 4));
  picklebuttThrustOutAnimation.looping = false;
  cloudAnimation = loadAnimation(...getAssetList('stars', 19));
  picklebuttImage = loadImage(`${basePath}assets/jelly_thrust_01.png`);
...
async function drawMessages() {
  if (paused) {
    ...
    text(
      gameOver
        ? `Your Jellycube ${splashDown ? 'missed the moon lmao' : `${tooFastToLand ? 'hit the moon too fast' : ''
